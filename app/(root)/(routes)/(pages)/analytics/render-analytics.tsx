...
const ClientComponent: React.FC<ClientComponentProps> = ({ initialVideoData, initialChannelData }) => {
  ...
  useEffect(() => {
    const videoDataCache = localStorage.getItem('videoData');
    const channelDataCache = localStorage.getItem('channelData');
    fetchAnalytics(videoDataCache, channelDataCache).then((result: any) => {
      if (result.error) {
